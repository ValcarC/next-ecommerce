        ...
        {slides.map(slide => (
          <div className={`${slide.bg} w-screen h-full flex flex-col gap-16 xl:flex-row`} key={slide.id}>
            ...
            <div className='h-1/2 xl:w-1/2 flex xl:h-full flex-col items-center justify-center gap-8 2xl:gap-12 text-center'>
              ...
              <Link href={slide.url}><button className='rounded-md bg-netral-950 text-zinc-50 py-3 px-4'>SHOP NOW</button></Link>
            </div>
            {/* IMAGE CONTAINER */}
            <div className='h-1/2 xl:w-1/2 xl:h-full relative'>
              <Image src={slide.img} alt='img' fill sizes='100%' className='object-cover'/>
            </div>
          </div>
        ))}
      </div>
